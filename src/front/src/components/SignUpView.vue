<template>
	<div id="signUp">
		...
		<v-form ref="signUpForm" validate-on="input" v-model="isValid">
			<v-responsive>
				<v-text-field v-model="user_data.id" label="아이디*" type="text" :rules="[rules.required,rules.lengthCheck]"/>
				<v-text-field ref="refPassword" v-model="user_data.password" label="새 비밀번호*" type="password" autocomplete="on" @input="validPassword" :rules="[rules.required,rules.lengthCheck,rules.passwordDoubleCheck]"/>
				<v-text-field ref="refPasswordConfirm" v-model="check_password" label="새 비밀번호 확인*" type="text" autocomplete="on" @input="validPassword" :rules="[rules.required,rules.lengthCheck,rules.passwordDoubleCheck]"/>
				<v-text-field v-model="user_data.name" label="이름*" type="text" :rules="[rules.required]"/>
				<v-text-field v-model="user_data.email" label="이메일*" type="email" :rules="[rules.required,rules.emailCheck]"/>
				<v-text-field v-model="user_data.phone" label="핸드폰번호*" type="phone" :rules="[rules.required,rules.phoneCheck]"/>
...
<script setup>
	...
	const signUp = async(user_data) => {
		await proxy.$store.dispatch('signUp',user_data);
		console.log(proxy.$store.getters.getResult);
		if(proxy.$store.getters.getResult === true){
			router.push('/site');
		}else{
